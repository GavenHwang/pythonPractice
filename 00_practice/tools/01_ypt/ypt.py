...
def login(phone, password):
    ...
    if res.json().get("code") != 1:
        raise ValueError(res.json().get("msg"))
    else:
        print("%s登陆成功！" % phone)
...
def get_args():
    ...
    parser.usage = "ypt定时抢购"
    parser.epilog = """
用法示例：
1.查询用户待卖商品列表：
python3 ypt.py -phone='12345678910' -password='111111' -c_time='19:30' -get_sells=True
2.指定优先购买的商品IDs和商品数量，商品的ids可通过步骤1查询获得
python3 ypt.py -phone='12345678910' -password='111111' -c_time='19:30' -start_time='19:27' -need_num=4 -goods_id='1,2,3'
    """
    parser.add_argument('-phone', type=str, dest='phone', help='电话，必填')
    parser.add_argument('-password', type=str, dest='password', help='密码，必填')
    ...
    parser.add_argument('-need_num', type=int, dest='need_num', help='需要多少个', default=1)
    parser.add_argument('-goods_id', type=str, dest='goods_id', help='指定要购买的商品ids')
    parser.add_argument('-get_sells', type=bool, dest='get_sells', help='为True时表示查询用户的待售商品列表', default=False)
    args = parser.parse_args()
    # 参数为空，直接退出
    if args.phone is None or args.password is None or args.start_time is None:
        parser.print_help()
        exit()
    return args
